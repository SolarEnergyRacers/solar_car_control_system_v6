...
public:
   ...
   */
  GlobalTime(I2CBus *p_i2cBus, const RtcDateTime *datetime = nullptr) : i2cBus(p_i2cBus) {
    this->_datetime = datetime ? *datetime : RtcDateTime(0);
    mutex = xSemaphoreCreateMutex();
  }

  int init(DS1307SquareWaveOut sq_freq, bool do_run);
  int re_init(DS1307SquareWaveOut sq_freq, bool do_run) { return init(sq_freq, do_run); }
