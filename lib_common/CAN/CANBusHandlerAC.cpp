...
//
#ifndef ACMODE
#pragma message "No AC-Mode --> not compiled"
#else
#pragma message "AC-Mode --> compiling..."
                ...
                if (ConfirmClearTime > 0) {
                    if (millis() > ConfirmClearTime) {
                        ...
                        if (tail(carState.DriverInfo, confirmation.length()) != confirmation) {
                            console << "DriverInfo '" << std::regex_replace(carState.DriverInfo, std::regex("\n+"), ", ") << "' confirmation received, appending: '" << confirmation << "'" << NL;
